...
### 2026-08-12 — Система работает end-to-end, обновлена документация

**Что сделано:**
- Подтверждена работа MVP на реальном ящике Яндекса (`janadolgaya@yandex.ru`): входящие письма тянутся (UID-синхронизация), исходящие уходят по SMTP (`250 Ok: queued`), копии пишутся в «Отправленные».
- Создан `docs/HANDOFF.md` — краткая передача контекста для старта новой сессии.
- `TASKS.md` — добавлен «Шаг 14. Интеграция с реальной почтой — runtime-фиксы ✅».
- `.dev-notes.md` — актуализированы «Грабли» (cron .mjs + TS, seen:false, 554 5.7.1, SMTP debug, curl), «Идеи на будущее» (почта отмечена done).
- `README.md` — раздел «Подключение почты» описывает UID-синхронизацию, append в «Отправленные» и ограничение `554 5.7.1`.

**Проблемы:**
- Отправка ответа на рассылочный/no-reply адрес отклоняется Яндексом (`554 5.7.1 ... restricted`) — политика провайдера, не баг кода. Отправка себе/обычным адресатам работает.

**Статус:** основной функционал CRM работает. Дальнейшие задачи — см. `docs/HANDOFF.md` (SMTP-лог под env, кнопка «Синхронизировать сейчас», рендер bodyHtml, обработка all-rejected, бэкапы, Telegram-уведомления).
...
**Что сделано:**
- `prisma/schema.prisma`: добавлено поле `lastSyncedUid Int?` на `Mailbox`
- Миграция `20260812000001_add_last_synced_uid` (один `ALTER TABLE ... ADD COLUMN`)
- `lib/email-sync.ts` — переработана `syncMailbox`:
  - Убран фильтр `{ seen: false }` (старая грабля: CRM качала только непрочитанные, поэтому прочитанные в веб-почте письма никогда не попадали в систему)
  - Запрос писем по UID-диапазону `${fromUid}:*` через `client.fetch(range, query, { uid: true })`
  - Первый запуск (`lastSyncedUid` пуст): тянется последние 50 писем (`uidNext - 50`)
  - Инкрементально: всё с `UID > lastSyncedUid`
  - CRM больше НЕ помечает письма `\Seen` при синхронизации (статус «прочитано» управляется пользователем)
  - `lastSyncedUid` сохраняется как watermark после синхронизации
  - Добавлены логи `[SYNC]` (подключение, статус INBOX, диапазон выборки, кол-во, итог) — видны в `docker compose logs app`

**Проблемы:**
- После запуска сервиса в CRM не было входящих писем, хотя IMAP-подключение работало (исходящие тесты проходили).
- Корневая причина: `syncMailbox` запрашивала только непрочитанные (`{ seen: false }`). Любое письмо, открытое в веб-почте/клиенте, CRM не подтягивала; плюс CRM сама помечала письма прочитанными, поэтому повторно их не видела.

**Решение:**
- Переход на инкрементальную синхронизацию по UID с watermark `lastSyncedUid`. Дедуп по `messageId` остаётся как подстраховка.
- `tsc --noEmit` и `npm run build` проходят.

**Изменённые файлы:**
- prisma/schema.prisma — поле lastSyncedUid
- prisma/migrations/20260812000001_add_last_synced_uid/migration.sql — новая миграция
- lib/email-sync.ts — UID-based sync + логирование

**Статус:** код готов; требуется пересборка Docker (`docker compose up --build -d`). Миграция применится автоматически при старте (`prisma migrate deploy` в entrypoint). Проверка: `docker compose logs app` (ищите `[SYNC]`).
